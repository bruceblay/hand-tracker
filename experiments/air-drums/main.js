...
function drawZones() {
  ...
  const labels = [
    { text: 'CRASH',  key: 'crash', x: w * 0.25, y: h * 0.25 },
    { text: 'HI-HAT', key: 'hihat', x: w * 0.75, y: h * 0.25 },
...
function handleHand(hand, side, drums) {
  ...
  if (p.justClosed) {
    ...
    if (high && !rightHalf)      { drums.crash(gain); flashes.crash = 1; }
    else if (high && rightHalf)  { drums.hihat(gain); flashes.hihat = 1; }
